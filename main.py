...
def projection_loss(online, target):
    norm_inner_prod = torch.div(torch.sum(online*target, dim=-1), (torch.linalg.norm(online, dim=-1) * torch.linalg.norm(target, dim=-1)))
    loss = (2 - (2 * norm_inner_prod)).mean()
    return loss
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(train_loader, desc=f"Training Epoch {epoch+1}") as bar:
            for batch_idx, batch in enumerate(bar):
                ...
                loss = je_loss(predictions[:, 1:], target_reps[:, 1:]) + projection_loss(online_preds, targets)
