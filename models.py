...
from encs import ResNet, build_resnet, build_vit
...
class JEPAModel(nn.Module):
    def __init__(self, device="cuda", repr_dim=256, proj_dim=128, action_dim=2, momentum=0.5):
        super().__init__()
        self.device = device
        ...
        self.online_encoder = build_vit(input_channels=2, repr_dim=repr_dim).to(device)
        ...
        self.target_encoder = build_vit(input_channels=2, repr_dim=repr_dim).to(device)
        ...
        self.predictor = RNNPredictor(repr_dim=repr_dim, action_dim=action_dim).to(device)
    ...
    def forward(self, states, actions):
        ...
        if self.training:
            ...
            with torch.no_grad():
                targets = self.target_encoder(states.view(B * T, C, H, W)).view(B, T, -1)  # [B, T, D]
                targets = self.target_projector(targets)
    ...
    def predict_future(self, init_states, actions):
        ...
        #initial state
        current_repr = self.online_encoder(init_states[:, 0])  # [B, D]
